...
export interface MeasurementPersistenceConfig {
    storageType: 'localStorage' | 'sessionStorage' | 'indexedDB' | 'server' | 'file';
    autoSave?: boolean;
...
export class MeasurementPersistence {
    ...
    private async initializeServerStorage(): Promise<void> {
        ...
        try {
            const response = await fetch(`${this.config.serverConfig.baseUrl}/health`);
            if (!response.ok) {
                throw new Error(`Server health check failed: ${response.status}`);
            }
        } catch (error) {
            console.warn('⚠️ Server storage initialization failed, falling back to localStorage');
    ...
    public async createSession(name: string, description?: string): Promise<string> {
    ...
    public async saveSession(session: MeasurementSession): Promise<void> {
        ...
        try {
            ...
            session.metadata.measurementTypes = this.extractMeasurementTypes(session.measurements);
            ...
            const data = this.config.compression ? this.compressData(session) : session;
            const encryptedData = this.config.encryption ? this.encryptData(data) : data;
    ...
    public async loadSession(sessionId: string): Promise<MeasurementSession | null> {
        if (!this.isInitialized) {
            throw new Error('Measurement Persistence not initialized');
        }
        ...
        try {
            ...
            const decryptedData = this.config.encryption ? this.decryptData(data) : data;
            const session = this.config.compression ? this.decompressData(decryptedData) : decryptedData;
    ...
    public async deleteSession(sessionId: string): Promise<void> {
        ...
        try {
            ...
            if (session) {
                await this.createBackup(sessionId, session.measurements, 'beforeDelete');
            }
    ...
    public async saveMeasurements(measurements: AllMeasurementData[], sessionId?: string): Promise<void> {
    ...
    public async loadMeasurements(sessionId?: string): Promise<AllMeasurementData[]> {
        const targetSessionId = sessionId || this.currentSessionId;
    ...
    public async exportMeasurements(sessionId: string, format: 'json' | 'csv' | 'xml' = 'json'): Promise<string> {
        const session = await this.loadSession(sessionId);
        if (!session) {
    ...
    public async importMeasurements(data: string, format: 'json' | 'csv' | 'xml' = 'json'): Promise<string> {
        ...
        try {
            ...
            const sessionId = await this.createSession(`Imported ${format.toUpperCase()}`, `Imported ${measurements.length} measurements`);
    ...
    public async createBackup(sessionId: string, measurements: AllMeasurementData[], reason: MeasurementBackup['reason']): Promise<string> {
        ...
        try {
            ...
            if (this.currentSessionId) {
                ...
                if (measurements.length > 0) {
                    await this.createBackup(this.currentSessionId, measurements, 'beforeClear');
                }
            }
    ...
    private async saveToLocalStorage(sessionId: string, data: any): Promise<void> {
    ...
    private async saveToSessionStorage(sessionId: string, data: any): Promise<void> {
        const key = `measurement_session_${sessionId}`;
    ...
    private async clearIndexedDB(): Promise<void> {
        return new Promise((resolve, reject) => {
            ...
            request.onsuccess = () => {
                const db = request.result;
                const transaction = db.transaction(['sessions', 'backups'], 'readwrite');
    ...
    private async saveToServer(sessionId: string, data: any): Promise<void> {
        ...
        const response = await fetch(`${this.config.serverConfig.baseUrl}${this.config.serverConfig.endpoints.save}`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                ...(this.config.serverConfig.apiKey && { 'Authorization': `Bearer ${this.config.serverConfig.apiKey}` })
            },
    ...
    private async loadFromServer(sessionId: string): Promise<any> {
        ...
        const response = await fetch(`${this.config.serverConfig.baseUrl}${this.config.serverConfig.endpoints.load}/${sessionId}`, {
            headers: {
                ...(this.config.serverConfig.apiKey && { 'Authorization': `Bearer ${this.config.serverConfig.apiKey}` })
            }
        });
    ...
    private async deleteFromServer(sessionId: string): Promise<void> {
        ...
        const response = await fetch(`${this.config.serverConfig.baseUrl}${this.config.serverConfig.endpoints.delete}/${sessionId}`, {
            method: 'DELETE',
            headers: {
                ...(this.config.serverConfig.apiKey && { 'Authorization': `Bearer ${this.config.serverConfig.apiKey}` })
            }
        ...
        if (!response.ok) {
            throw new Error(`Server delete failed: ${response.status} ${response.statusText}`);
        }
    }
    ...
    private async listFromServer(): Promise<MeasurementSession[]> {
        ...
        const response = await fetch(`${this.config.serverConfig.baseUrl}${this.config.serverConfig.endpoints.list}`, {
            headers: {
                ...(this.config.serverConfig.apiKey && { 'Authorization': `Bearer ${this.config.serverConfig.apiKey}` })
            }
        });
        ...
        if (!response.ok) {
            throw new Error(`Server list failed: ${response.status} ${response.statusText}`);
        }
    ...
    private async loadBackup(backupId: string): Promise<MeasurementBackup | null> {
    ...
    private extractMeasurementTypes(measurements: AllMeasurementData[]): string[] {
    ...
    private convertToCSV(measurements: AllMeasurementData[]): string {
        ...
        const headers = ['ID', 'Tool Name', 'Value', 'Unit', 'Timestamp', 'Viewport ID', 'Image ID'];
    ...
    private convertToXML(session: MeasurementSession): string {
        ...
        xml += `  <description><![CDATA[${session.description || ''}]]></description>\n`;
...
export function createMeasurementPersistence(config: MeasurementPersistenceConfig): MeasurementPersistence {
