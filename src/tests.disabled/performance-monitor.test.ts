...
describe('PerformanceMonitor', () => {
  ...
  describe('Metrics History', () => {
    ...
    it('should filter metrics by time range', async () => {
      ...
      const filteredMetrics = performanceMonitor.getMetricsHistory(MetricCategory.RENDERING, {
        start: startTime,
        end: endTime,
      });
  ...
  describe('Event System', () => {
    ...
    it('should emit optimization recommended events', async () => {
      ...
      const onCallback = vi
        .mocked(viewportOptimizer.on)
        .mock.calls.find((call: any) => call[0] === 'performance-updated');
