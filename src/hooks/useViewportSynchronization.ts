...
import { SynchronizationSettings, ViewportSyncState, SyncType, SynchronizationEvent } from '../types/dicom';
...
export const useViewportSynchronization = ({
  ...
  const [syncSettings, setSyncSettings] = useState<SynchronizationSettings>(viewportSynchronizer.getSyncSettings());
  ...
  useEffect(() => {
    ...
    const handleSynchronizationApplied = (event: SynchronizationEvent) => {
      ...
      if (
        event.targetViewportIds.includes(viewportId) &&
  ...
  const setWindowLevel = useCallback(
    (center: number, width: number, triggerSync: boolean = true) => {
      if (!isRegistered) return;

      isUpdatingRef.current = true;

      viewportSynchronizer.updateViewportState(
        viewportId,
        {
          windowCenter: center,
          windowWidth: width,
  ...
  const setZoom = useCallback(
    (zoomLevel: number, triggerSync: boolean = true) => {
      if (!isRegistered) return;
  ...
  const setPan = useCallback(
    (panValue: [number, number], triggerSync: boolean = true) => {
      if (!isRegistered) return;
  ...
  const setScrollIndex = useCallback(
    (index: number, triggerSync: boolean = true) => {
      if (!isRegistered) return;
  ...
  const setOrientation = useCallback(
    (orientationValue: 'AXIAL' | 'SAGITTAL' | 'CORONAL', triggerSync: boolean = true) => {
      if (!isRegistered) return;
  ...
  const triggerSync = useCallback(
    (syncType: SyncType, data?: Record<string, unknown>) => {
  ...
  // State queries
  const isInSyncGroup = useCallback(
    (groupId: string) => {
